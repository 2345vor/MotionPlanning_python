...
class C:
    ...
    MAX_STEER = 1  # [rad] maximum steering angle
...
def main_Crusing():
    ...
    s0_a = 0.0
    target_speed_adjusted = False  # 添加一个变量来跟踪目标速度是否已调整
    add = 0.1  # 用于调整目标速度
    ...
    while True:
        ...
        if path is None:
            ...
            add += 0.1  # 调整目标速度
            if not target_speed_adjusted:
                C.TARGET_SPEED += add  # 第一次加 0.5
                target_speed_adjusted = True  # 标记为已调整
            else:
                C.TARGET_SPEED -= add  # 第二次减 0.5
                target_speed_adjusted = False  # 重置标记
